...
interface ILottery{
    ...
    function buyLottery(address user,uint256[6][] calldata values) external;
...
contract Lottery is ILottery,IRandomCaller,Ownable,ReentrancyGuard,NotContract{
    RandomNumberConsumer public Randomer;
    IERC20 public LabsToken;

    address public FeeReceiver = address (0x19e5A6852aCBF8B80D07f65A6Dc3336E85512b4d);
    ...
    function withdrawLabs() external onlyOwner{
        LabsToken.transfer(owner(),LabsToken.balanceOf(address(this)));
    }
    ...
    function lottery() public override {
    ...
    function setRandomer(address randomer) public {
        Randomer = RandomNumberConsumer(randomer);
    }

    function settlementLottery() public override {
        Randomer.generateLotteryNumber(SettlementRound);
    ...
    function _settlementLottery(uint256 roundNumber) internal  {
        ...
        uint256 number2 = (number1 * (10 ** 2)) + winningNumber[1];
        uint256 number3 = (number2 * (10 ** 2)) + winningNumber[2];
        uint256 number4 = (number3 * (10 ** 2)) + winningNumber[3];
        uint256 number5 = (number4 * (10 ** 2)) + winningNumber[4];
        uint256 number6 = (number5 * (10 ** 2)) + winningNumber[5];
        ...
        if(Lotteries[roundNumber].WinnersCount[6] != 0){
            amount += ((Lotteries[roundNumber].PrizePool) * 40 / 100);
        }

        if(Lotteries[roundNumber].WinnersCount[5] != 0){
            amount += ((Lotteries[roundNumber].PrizePool) * 26 / 100);
        }

        if(Lotteries[roundNumber].WinnersCount[4] != 0){
            amount += ((Lotteries[roundNumber].PrizePool) * 15 / 100);
        }

        if(Lotteries[roundNumber].WinnersCount[3] != 0){
            amount += ((Lotteries[roundNumber].PrizePool) * 10 / 100);
        }

        if(Lotteries[roundNumber].WinnersCount[2] != 0){
            amount += ((Lotteries[roundNumber].PrizePool) * 6 / 100);
        }

        if(Lotteries[roundNumber].WinnersCount[1] != 0){
        ...
        Lotteries[roundNumber + 1].PrizePool += Lotteries[roundNumber].PrizePoolRemainder;
    ...
    function buyLottery(address user,uint256[6][] calldata numbers) public notContract nonReentrant override {
        require(Lotteries[CurrentRound].Status == lotterystatus.Open,"The current round is not started or has ended!");
        ...
        for(uint8 i = 0; i < numbers.length; i++){
            ...
            uint256 index1 = (index0 * (10 ** 2)) + numbers[i][1];
            Lotteries[CurrentRound].CountOfTicket[index1]++;

            uint256 index2 = (index1 * (10 ** 2)) + numbers[i][2];
            Lotteries[CurrentRound].CountOfTicket[index2]++;

            uint256 index3 = (index2 * (10 ** 2)) + numbers[i][3];
            Lotteries[CurrentRound].CountOfTicket[index3]++;

            uint256 index4 = (index3 * (10 ** 2)) + numbers[i][4];
            Lotteries[CurrentRound].CountOfTicket[index4]++;

            uint256 index5 = (index4 * (10 ** 2)) + numbers[i][5];
    ...
    function claimThePrize(uint256 roundNumber) public notContract nonReentrant override{
        ...
        LabsToken.transfer(FeeReceiver, fee);
        LabsToken.transfer(msg.sender, amount - fee);
